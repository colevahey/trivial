            ...
            <div key={`${node.type}-${node.id}-${index}`} className="flex items-center">
              ...
              >
                {node.type === 'actor' ? (
                  ...
                  <div className="flex flex-col items-center gap-2">
                    <div className="w-14 h-20 rounded-lg overflow-hidden ring-1 ring-zinc-700 bg-zinc-800">
                      {node.image ? (
                        <Image
                          src={node.image}
                          alt={node.name}
                    ...
                    <div className="text-center max-w-[72px]">
                      <div className="text-zinc-400 text-[10px] leading-tight line-clamp-2">{node.name}</div>
                      {node.character && (
                        <div className="text-zinc-600 text-[9px] italic mt-0.5 line-clamp-1">as {node.character}</div>
                      )}
                    </div>
                  </div>
                )}
              ...
              {index < path.length - 1 && (
                <div
                  className={`flex items-center px-1 transition-all duration-500 ${
