...
export default function ActorPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const avgRating = credits.length > 0
    ? credits.filter(m => m.vote_average > 0).reduce((s, m) => s + m.vote_average, 0) /
      credits.filter(m => m.vote_average > 0).length
    : 0

  const activeYears = sortedCredits.length > 0
    ? `${new Date(sortedCredits[sortedCredits.length - 1].release_date).getFullYear()} – ${new Date(sortedCredits[0].release_date).getFullYear()}`
    : '—'
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      {topFilms.length > 0 && (
        <div className="mb-10">
          <h2 className="text-lg font-bold text-white mb-4">Top Rated Films</h2>
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-3">
            {topFilms.map((film: Movie) => (
              <FilmCard key={film.id} movie={film} showCharacter />
            ))}
          </div>
        </div>
      )}
        ...
        <div className="p-6">
          {activeTab === 'timeline' && (
            <div>
              ...
              <p className="text-zinc-500 text-xs mb-4">Film rating over time. Dot size represents box office revenue. Color: gold = 7+, purple = 5–7, gray = below 5.</p>
          ...
          {activeTab === 'genres' && (
            <div>
              ...
              <p className="text-zinc-500 text-xs mb-4">Distribution of genres across all films in this actor&apos;s career.</p>
          ...
          {activeTab === 'boxoffice' && (
            <div>
              <h3 className="text-white font-semibold mb-1">Box Office vs Rating</h3>
              <p className="text-zinc-500 text-xs mb-4">Annual box office revenue (bars) and average rating (line) over time.</p>
          ...
          {activeTab === 'filmography' && (
            <div>
              ...
              <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-3 max-h-[500px] overflow-y-auto pr-2">
                {sortedCredits.map((film: Movie) => (
                  <FilmCard key={film.id} movie={film} showCharacter />
                ))}
              </div>
